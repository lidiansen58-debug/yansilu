...
- 2026-05-09 follow-up: literature notes now include a citation metadata section and block `记录永久笔记` until required citation fields are present.
...
- Literature note citation metadata is covered in the note template, parser, product docs, and e2e. Required fields before `记录永久笔记`: citation title, author, year, locator, and at least one DOI/ISBN/arXiv/URL/PDF-style trace identifier.
