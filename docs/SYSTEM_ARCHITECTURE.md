...
### 6.1 Simplified Obsidian Importer

能力：

- 导入 Obsidian vault 中的 Markdown 文件与目录
- 识别 frontmatter
- 识别 wikilinks
- 识别 tags / aliases
- 复制嵌入资源到 vault 内可用路径
- 通过 preview -> confirm 写入来源卡片、文献笔记、永久笔记候选

### 6.2 Future Connector Slots

当前版本不把 Zotero、Readwise、NotebookLM 等外部连接器纳入发布门槛。相关能力应视为后续阶段规划，而不是现行产品承诺。
...
- 导入确认：`POST /api/v1/imports/:id/confirm`
- 导入回滚：`POST /api/v1/imports/:id/rollback`（当前仅返回 unsupported）
