  ...
  if (!filtered.length) {
    return `
      <div class="graph-thinking-empty">
        ...
        <span>${escapeHtml(meta.key === "theme" ? "运行图谱扫描后，可以在这里查看可能成题的主题群。" : "继续写笔记、补关系说明或运行图谱扫描后，这里会出现新的待处理内容。")}</span>
        ...
        ? (
            categories.length
              ? `<div class="graph-thinking-categories">${categories
                  ...
                  .join("")}</div>`
              : `<div class="graph-empty">当前范围暂时没有明显要优先判断的内容。可以继续写笔记或补充关系说明后再刷新图谱。</div>`
          )
        : ""
    }
