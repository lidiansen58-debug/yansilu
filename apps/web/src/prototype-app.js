...
function renderExplorerSidebarFlow(rootId = state.browserRootId) {
  ...
  const metrics = isOriginal
    ? permanentRouteValueMetrics(originalNotes)
    : [
  ...
  if (reason === "save-note-distillation") {
    ...
    try {
      const updated = await updateNote(note.id, {
        ...
        authorship: payload.authorship || note.authorship || undefined
      });
      if (updated) {
        ...
        if (payload.authorship) {
          note.authorship = normalizeAuthorshipItem(payload.authorship) || note.authorship;
        }
      }
