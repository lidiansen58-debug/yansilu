...
function systemMessageActionLabel(message = {}) {
  ...
  if (message.action === "open-graph") return "回到图谱";
...
async function pullRecommendedOllamaModel() {
  ...
  try {
    ...
    if (result?.enabled?.preferences) {
      ...
      settingsState.ai.localModel = selectedLocalModelNameForInstalledModels(modelName, models);
    }
    ...
    setStatus(
      installedLocalModelReady(readyModel)
        ? `本地模型已就绪：${readyModel}`
        : `模型下载已完成，但还没有在 Ollama 列表里检测到 ${modelName}。请稍后重新检测。`,
...
function renderAiRoutePreview() {
  ...
  if (settingsState.ai.routePreviewError) {
    ...
    if (localReady) {
      ...
      detail.innerHTML = `
        <div class="settings-route-preview-title">本地模型已就绪</div>
        <div class="settings-route-preview-copy">当前会使用 Ollama 本地，模型为 ${escapeHtml(localModel)}。</div>
        <div class="settings-route-preview-action">下一步：用一句不含敏感内容的话试运行，确认回复正常。</div>
      `;
      return;
    }
  ...
  const routeModelName = modelRef.includes(":")
    ...
    : modelRef;
  const localModel = String(settingsState.ai.localModel || "").trim();
  const displayModel = route.localOnly
  ...
  const statusText = route.localOnly
    ? (ready ? "本地模型已就绪" : "本地模型待设置")
    : (ready ? "远程模型已配置" : "远程模型待设置");
  const statusTone = ready ? "ok" : "warn";
  const actionText = ready
    ? "下一步：用一句不含敏感内容的话试运行，确认回复正常。"
    : route.localOnly
      ? "下一步：启动 Ollama，并选择或下载一个本地模型。"
  ...
  detail.innerHTML = `
    <div class="settings-route-preview-title">${escapeHtml(statusText)}</div>
    <div class="settings-route-preview-copy">当前会使用 ${escapeHtml(serviceLabel)}，模型为 ${escapeHtml(displayModel)}。</div>
    <div class="settings-route-preview-copy">AI 方案：${escapeHtml(modelPackDisplayLabel(preview.modelPack || settingsState.ai.modelPack || "Starter Auto"))}</div>
    ${localRuntimeLine}
...
function renderAiLocalModelRecommendations() {
  ...
  const installedCount = OLLAMA_MODEL_RECOMMENDATIONS.filter((item) => hasLocalModel(item.name)).length;
  const summaryText = installedCount ? "可选下载" : "建议先下载";
  recommendationsEl.innerHTML = `
    <div class="settings-ai-local-recommendations-head">
      <span>${escapeHtml(summaryText)}</span>
    </div>
    <div class="settings-ai-local-recommendation-list">
      ${OLLAMA_MODEL_RECOMMENDATIONS.map((item) => {
        const installed = hasLocalModel(item.name);
        return `
          <div class="settings-ai-local-recommendation ${installed ? "is-installed" : ""}">
            <strong>${escapeHtml(item.name)}</strong>
            <span>${escapeHtml(item.label)}</span>
            <div class="settings-ai-local-recommendation-status">${installed ? "已安装" : "未安装"}</div>
          </div>
        `;
      }).join("")}
    </div>
...
const GRAPH_RELATION_GROUP_META = {
  support: { label: "支持系", detail: "支持 / 补充 / 推进 / 例子" },
  conflict: { label: "张力系", detail: "反驳 / 对比 / 反例" },
  boundary: { label: "边界系", detail: "限定" },
  bridge: { label: "桥接系", detail: "桥接 / 追问 / 意外相关 / 改写问题" },
  flow: { label: "过程系", detail: "前提 / 后续 / 进入草稿" },
  neutral: { label: "链接线索", detail: "同主题 / 重述 / 自由链接 / 重复重叠" },
  index: { label: "索引结构", detail: "归属主题" }
};
...
function graphFocusDepthMeta(value = "") {
  ...
  if (key === "2") return { key, label: "2 跳", note: "往外再看一层论证邻居" };
  if (key === "all") return { key, label: "整块相连", note: "看这条笔记所在的整块连通结构" };
  return { key: "1", label: "1 跳", note: "只看直接相连的一跳关系" };
}
...
function graphFocusContextModeMeta(value = "") {
  ...
  if (key === "writing") {
    return {
      key,
      label: "写作阅读",
      note: "优先看桥接、进入草稿、前提和后续，判断这条笔记怎么进入文章。"
    };
  }
  return {
    key: "argument",
    label: "论证阅读",
    note: "优先看支持、反驳和限定，判断这条笔记的论证位置是否清楚。"
  };
}
  ...
  return `
    <section class="graph-orientation" aria-label="图谱读法">
      <div class="graph-orientation-main">
        ...
        <span>节点不是文件数量，边也不是装饰线。它们一起回答：中心观点在哪里，哪些笔记在支撑，哪里有反方、边界或桥接缺口。下面这组数字按当前目录的全部结构统计，不受上方关系筛选影响。</span>
      </div>
      ...
      <div class="graph-orientation-metrics">
        <span>总节点 ${Number(nodes.length || 0)}</span>
...
function buildGraphInsightCoach({ nodes = [], edges = [], conflictItems = [], bridgeGaps = [], untypedRelations = [] } = {}) {
  ...
  const thesis = !nodes.length
    ...
    : !edges.length
      ? bridgeGaps.length
        ? `当前至少还有 ${bridgeGaps.length} 个桥接缺口。先把孤立笔记或断裂簇连回主结构，再谈图谱阅读和写作路径。`
  ...
  const prompts = !edges.length
    ? [
        ...
        bridgeGaps.length ? `先从 ${bridgeGaps[0]?.noteTitles?.[0] || "当前孤立笔记"} 开始，给它补一条能回到主结构的桥接关系。` : "先挑两条笔记，写出明确的支持、限定或反驳关系。",
        "桥接写清后，再回来看哪些关系理由还偏薄。"
      ]
    : [
...
function resolveGraphIsolatedSelection(selection = null, isolatedNotes = [], nodes = []) {
  ...
  if (matchIndex >= 0) {
    ...
    return {
      ...
      title: String(note?.title || resolvedNoteId || "孤立笔记").trim() || "孤立笔记"
    };
  }
  if (noteId && nodes.some((node) => String(node?.id || "").trim() === noteId)) {
    ...
    return {
      ...
      isolatedKey: noteId,
      noteId,
      title: String(node?.title || noteId || "孤立笔记").trim() || "孤立笔记"
    };
  }
  return null;
}
...
function resolveGraphBridgeSelection(selection = null, bridgeGaps = [], nodes = []) {
  ...
  return {
    ...
    title: String(gap?.noteTitles?.[0] || nodeMap.get(sourceId)?.title || sourceId || "桥接机会").trim() || "桥接机会",
...
function graphNodeRoleMeta(node = {}, directEdges = []) {
  ...
  if (!degree) {
    return {
      label: "孤立笔记",
      tone: "isolated",
      detail: "它暂时还没有靠近任何主题星系。先判断：这是值得保留的独立观察，还是缺少一条桥接关系。",
      prompt: "它应该保持独立成远星，还是能连接到一个正在形成的主题？"
    };
  }
...
function graphNodeInsightMeta(node = {}, directEdges = [], { nodeMap = new Map(), edges = [] } = {}) {
  ...
  const themeNoteIds = graphThemeCandidateNoteIdsForNode(noteId, directEdges, aiCandidates);
  let quality = "关系可读";
...
function renderGraphNodeInsightPanel(insight = {}) {
  ...
  const rows = [
    { label: "这条笔记的位置", value: insight.position, detail: insight.positionDetail },
    { label: "关系质量", value: insight.quality, detail: insight.qualityDetail },
    { label: "下一步", value: insight.nextStep, detail: `${Number(insight.candidateCount || 0)} 条候选 · ${Number(insight.themeNoteCount || 0)} 条可成题笔记` }
  ];
  return `
    <section class="graph-node-insight" aria-label="节点洞见">
      ${rows
        .map(
          (row) => `
            <div>
...
function renderGraphSelectionShell({ className = "", ariaLabel = "", kicker = "", title = "", meta = "", closeLabel = "收起详情", roleLabel = "", roleDetail = "", body = "", actions = "" } = {}) {
...
function renderGraphThemeSelectionPanel({ selection = null, topicCandidates = [], nodeMap = new Map(), edges = [] } = {}) {
  ...
  return renderGraphSelectionShell({
    ...
    kicker: "主题簇成熟度",
    title: theme.title,
    meta: `${maturity.noteIds.length || 0} 条笔记 · ${maturity.memberEdges.length || 0} 条簇内关系`,
    closeLabel: "收起主题簇详情",
    ...
    body: `
      ...
      <div class="graph-selection-metrics" aria-label="主题簇证据结构">
        ${renderGraphSelectionMetrics([
          { label: "成员", value: `${maturity.noteIds.length} 条` },
          { label: "簇内关系", value: `${maturity.memberEdges.length} 条` },
          { label: "支撑", value: `${maturity.counts.support || 0} 条` },
          { label: "边界/张力", value: `${(maturity.counts.boundary || 0) + (maturity.counts.conflict || 0)} 条` }
        ])}
      </div>
...
function openGraphIsolatedDecisionAction(noteId = "", action = "") {
  ...
  editor?.renderRelated?.("孤立笔记整理");
  const messages = {
    keep: "已打开孤立笔记：请补一句“为什么暂时保持独立”，避免之后误删或硬连线。",
    hold: "已打开孤立笔记：先把它标成暂存观察，等形成判断后再进入关系图谱。",
    rewrite: "已打开孤立笔记：请先重写中心判断，再决定是否补关系。"
  };
  setStatus(messages[cleanAction] || "已打开孤立笔记，继续整理它的角色。", cleanAction === "hold" ? "warn" : "ok", {
    priority: 2,
    holdMs: 3600,
...
function renderGraphIsolatedQueueStrip({ isolatedNotes = [], nodeMap = new Map(), edges = [], currentNoteId = "", queueItems: providedQueueItems = null } = {}) {
  ...
  const total = Array.isArray(isolatedNotes) ? isolatedNotes.length : queueItems.length;
  return `
    <div class="graph-isolated-queue-strip" aria-label="孤立笔记连续整理入口">
      <div>
...
function graphAiRelationCandidatesForNote(noteId = "", { nodeMap = new Map(), edges = [], limit = 5 } = {}) {
  ...
  return rawCandidates
    .map((candidate = {}) => {
      ...
      const toNoteId = String(candidate.toNoteId || candidate.to_note_id || candidate.targetNoteId || candidate.target_note_id || candidate.to?.id || "").trim();
...
function renderGraphRelationCandidateCards(candidates = [], { title = "可能相关笔记", note = "先复查这些线索，再把能说清理由的关系写入图谱。" } = {}) {
...
function renderGraphAiConnectCandidates(noteId = "", { nodeMap = new Map(), edges = [] } = {}) {
  const candidates = graphAiRelationCandidatesForNote(noteId, { nodeMap, edges, limit: 5 });
          ...
          .map(
            (candidate) => {
              ...
              return `
                ...
                ${renderGraphCandidateReviewRows(candidate)}
                <div class="graph-ai-connect-actions">
...
function renderGraphRelationWorkspaceForNote(noteId = "", { nodeMap = new Map(), edges = [], title = "关联整理" } = {}) {
  ...
  const counts = graphRelationGroupCounts(directEdges);
  const aiCandidates = graphAiRelationCandidatesForNote(cleanNoteId, { nodeMap, edges, limit: 5 });
  const localCandidates = graphLocalRelationCandidatesForNote(cleanNoteId, { nodeMap, edges, limit: 5 });
  const themeNoteIds = graphThemeCandidateNoteIdsForNode(cleanNoteId, directEdges, aiCandidates);
...
function renderGraphIsolatedJoinNetworkFlow(noteId = "", { nodeMap = new Map(), edges = [], visibleEdgeCount = 0 } = {}) {
  const cleanNoteId = String(noteId || "").trim();
  if (!cleanNoteId) return "";
  ...
  const stateLabel = directEdges.length
    ? "已有关系"
    : aiCandidates.length
      ? `${aiCandidates.length} 条候选`
      : localCandidates.length
        ? `${localCandidates.length} 条线索`
      : hasAnalysis
        ? "待手工判断"
        : "待连接";
  const candidatePreview = [...aiCandidates, ...localCandidates]
    .map((candidate) => candidate.counterpartTitle || candidate.targetTitle)
    .filter(Boolean)
    .slice(0, 2)
    .join("、");
  ...
  const steps = [
    {
      title: "发现候选",
      detail: candidatePreview ? `优先复查 ${candidatePreview}${aiCandidates.length > 2 ? " 等候选" : ""}。` : "先用 AI 或手工搜索找一条可能成立的连接。",
      state: aiCandidates.length ? "done" : loading ? "current" : "idle"
    },
    {
      title: "确认关系理由",
      detail: "只把能说清楚理由的支持、限定、反驳或桥接写入图谱。",
      state: directEdges.length ? "done" : aiCandidates.length ? "current" : "idle"
    },
    {
      title: "回到网络阅读",
      detail: visibleEdgeCount ? `当前可见 ${visibleEdgeCount} 条关系，可以继续看一跳结构。` : "保存关系后，这条笔记会从孤立状态进入网络阅读。",
      state: directEdges.length ? "current" : "idle"
    }
  ];
  return `
    <section class="graph-isolated-join" aria-label="孤立笔记关联到图谱">
      ...
      <div class="graph-isolated-join-steps" aria-label="关联到图谱步骤">
        ${steps
          .map(
            (step, index) => `
              <div class="graph-isolated-join-step is-${escapeHtml(step.state)}">
                <span>${index + 1}</span>
                <div>
                  <strong>${escapeHtml(step.title)}</strong>
...
function renderGraphIsolatedNextStepActions(noteId = "", { isolatedNotes = [], nodeMap = new Map(), edges = [] } = {}) {
  ...
  const themeNoteIds = graphThemeCandidateNoteIdsForNode(cleanNoteId, directEdges, aiCandidates);
  const themeTitle = suggestedThemeIndexTitle(themeNoteIds);
  const canCreateTheme = directEdges.length > 0 && themeNoteIds.length >= 2;
  return `
    <section class="graph-isolated-next-step" aria-label="保存关系后的下一步">
      <div>
        <strong>确认关系后继续</strong>
        <p>${escapeHtml(nextItem ? `下一条优先处理：“${nextItem.title}”。` : "当前队列暂时没有下一条孤立笔记，可以回到这条笔记周边继续阅读。")}</p>
      </div>
...
function renderGraphIsolatedSelectionPanel({ selection = null, isolatedNotes = [], nodeMap = new Map(), edges = [] } = {}) {
  ...
  const title = String(isolated.title || note?.title || noteId || "孤立笔记").trim() || "孤立笔记";
  ...
  return renderGraphSelectionShell({
    ...
    kicker: "孤立笔记",
    title,
    meta: `${noteTypeLabel(note.noteType)} · 当前可见关系 ${visibleEdgeCount} 条`,
    closeLabel: "收起孤立笔记整理",
    roleLabel: decision.label,
    roleDetail: decision.detail,
    body: `
      ${renderGraphIsolatedJoinNetworkFlow(noteId, { nodeMap, edges, visibleEdgeCount })}
      ${renderGraphIsolatedNextStepActions(noteId, { isolatedNotes, nodeMap, edges })}
      ${isolatedQueueMarkup}
      <section class="graph-selection-reason">
        <small>当前判断</small>
        <p>${escapeHtml(thesis || "这条笔记还没有明显的中心判断。先把自己的判断写出来，再决定要不要建立关系。")}</p>
      </section>
      <div class="graph-isolated-decisions" aria-label="孤立笔记处理方式">
        ${decisionCards
          .map(
            (card) => `
              <button class="graph-isolated-decision${card.active ? " is-active" : ""}" type="button" data-graph-isolated-action="${escapeHtml(card.key)}" data-open-note="${escapeHtml(noteId)}" aria-pressed="${card.active}">
...
function renderGraphBridgeSelectionPanel({ selection = null, bridgeGaps = [], nodeMap = new Map() } = {}) {
  ...
  const gapTypeLabel = bridge.gapType === "disconnected_cluster" ? "断裂簇" : "桥接机会";
  const detail = String(item?.suggestedAction || item?.rationale || "这条笔记可能需要一条中间判断，才能回到当前结构。").trim();
  const prompts = [
    targetTitle ? `它和「${targetTitle}」之间缺的是证据、限定、反方，还是一个中间概念？` : "它应当保持独立，还是只是缺少一条桥接关系？",
    "如果补上这条桥接，图谱会产生新的论证路径，还是只是多一条导航线？",
    "这条桥接关系应该写成概念过渡、方法相似，还是问题延伸？"
  ];
  return renderGraphSelectionShell({
    className: "is-bridge",
    ariaLabel: "桥接机会判断详情",
    ...
    actions: `
      ...
      <button class="graph-selection-action" type="button" data-open-note="${escapeHtml(noteId)}" data-graph-followup-action="bridge"${targetNoteId ? ` data-graph-target-note="${escapeHtml(targetNoteId)}"` : ""} data-graph-relation-type="bridges"${noteId ? "" : " disabled"}>补桥接关系</button>`
  });
}
...
function renderGraphClusterSelectionPanel({ selection = null, clusterMeta = [], nodeMap = new Map(), edges = [] } = {}) {
  ...
  return renderGraphSelectionShell({
    ...
    body: `
      ...
      ${renderGraphThemeIndexWorkspace(meta.memberIds, { title: normalized.title || cluster.title || "未命名星系", relationCount: meta.memberEdges.length, tone: meta.tone })}
    ...
    actions: `
      <button class="graph-selection-action is-primary" type="button" data-graph-create-theme-index data-graph-theme-note-ids="${escapeHtml(meta.memberIds.join(","))}" data-graph-theme-title="${escapeHtml(normalized.title || cluster.title || "未命名星系")}"${meta.memberIds.length >= 2 ? "" : " disabled"}>创建主题笔记</button>
...
function graphResearchNavigatorState({ nodes = [], edges = [], topicCandidates = [], bridgeGaps = [], clusterMeta = [], clueSummary = null, questionSummary = null } = {}) {
  ...
  const verdict = clusters.length
    ? promisingClusterCount
      ? `这批笔记聚成 ${clusters.length} 个主题星系，其中 ${promisingClusterCount} 个可以继续提炼成研究问题或文章判断。`
      : `这批笔记已经聚成 ${clusters.length} 个主题星系，但关系理由还偏薄，先不要急着定题。`
    : "这批笔记还没有明显主题星系，先从最亮的笔记补第一批关系。";
  const nextAction = clusters.length
    ? brightNodes.length
      ? `建议先点开 ${brightNodes.length} 颗优先亮星，判断它们分别承担“证据、反方、边界、桥接”里的哪种角色。`
  ...
  return `
    <aside class="graph-research-navigator" aria-label="图谱概览">
      ...
      <div class="graph-research-head">
        <span>图谱概览</span>
        <strong>先判断能不能成题</strong>
      </div>
      <section class="graph-research-verdict">
        <small>${escapeHtml(nav.headline)}</small>
        <strong>${escapeHtml(nav.verdict)}</strong>
        <p>${escapeHtml(nav.nextAction)}</p>
      </section>
      ...
      <section class="graph-research-next">
        ...
        <p>${escapeHtml(nav.pendingNote)}</p>
        <p>${escapeHtml(nav.risk)}</p>
      </section>
      ${
        clusterCards.length
          ? `<section class="graph-research-section">
              <strong>先看的主题星系</strong>
              ${clusterCards
                .map(
                  ({ cluster, meta }) => `
                    <button class="graph-research-card" type="button" data-graph-select-cluster="${escapeHtml(cluster.clusterKey)}" aria-label="查看主题星系 ${escapeHtml(cluster.title)}">
      ...
      ${
        nav.brightNodes.length
          ? `<section class="graph-research-section">
              <strong>优先点开的亮星</strong>
              ${nav.brightNodes
                .map(
                  (node) => `
                    <button class="graph-research-card" type="button" data-node-id="${escapeHtml(node.id)}" data-graph-select-node="${escapeHtml(node.id)}" aria-label="查看优先亮星 ${escapeHtml(node.title || node.id)}">
                      <span>${escapeHtml(node.title || node.id)}</span>
                      <small>连接 ${escapeHtml(String(node.degree || 0))} 条 · 先判断它是证据、反方、边界还是桥接</small>
                    </button>
                  `
                )
                .join("")}
...
function renderGraphSelectionPanel({ selection = null, nodeMap = new Map(), edges = [], topicCandidates = [], isolatedNotes = [], bridgeGaps = [], clusterMeta = [] } = {}) {
      ...
      body: `
        ...
        <div class="graph-selection-metrics" aria-label="节点关系分布">
          ${renderGraphSelectionMetrics([
            ...
            { label: "张力", value: String((counts.conflict || 0) + (counts.boundary || 0)) },
            { label: "桥接", value: String(counts.bridge || 0) },
            { label: "过程", value: String(counts.flow || 0) }
          ])}
        </div>
        <section class="graph-selection-prompts">
          <strong>接下来可以问</strong>
          ${prompts.map((prompt) => `<p>${escapeHtml(prompt)}</p>`).join("")}
        </section>`,
      actions: `
  ...
  const actionMeta = graphFocusCardActionMeta(edge, graphState.focusContextMode);
      ...
      <section class="graph-relation-adjustment" aria-label="关系调整建议">
        ...
        <div class="graph-relation-adjustment-grid">
          ${adjustment.cards
            .map(
              (card) => `
                <button class="graph-relation-adjustment-card${card.active ? " is-active" : ""}" type="button" data-graph-relation-adjustment="${escapeHtml(card.key)}" data-open-note="${escapeHtml(sourceId)}" data-graph-relation-id="${escapeHtml(relationId)}"${targetId ? ` data-graph-target-note="${escapeHtml(targetId)}"` : ""}${relationType ? ` data-graph-relation-type="${escapeHtml(relationType)}"` : ""}>
                  <strong>${escapeHtml(card.title)}</strong>
                  <span>${escapeHtml(card.text)}</span>
                  <small>${escapeHtml(card.actionLabel)}</small>
                </button>
...
function graphNodeAttentionReasons(node = {}, { selected = false, inSelectedTheme = false, selectedIsolated = false, inSelectedBridge = false } = {}) {
  ...
  if (node.isGraphIsolatedCandidate || selectedIsolated) reasons.push("孤立笔记");
...
function renderGraphVisualMap({
  ...
  const nodeMarkup = layout.nodes
    .map((node, index) => {
      ...
      const metaLabel = node.isGraphIsolatedCandidate ? "孤立笔记" : noteTypeLabel(node.noteType);
                ...
                <div class="graph-focus-depth" aria-label="中心阅读深度">
                  ...
                  ${
                    focusContextAvailable
                      ? `<button class="graph-focus-panel-toggle" type="button" data-graph-focus-context-toggle="${focusContextCollapsed ? "open" : "close"}" aria-expanded="${focusContextCollapsed ? "false" : "true"}" aria-controls="graphFocusContextPanel" title="${focusContextCollapsed ? "显示右侧阅读" : "收起右侧阅读"}">${focusContextCollapsed ? "显示右侧阅读" : "收起右侧阅读"}</button>`
                      : ""
                  }
                </div>
              </div>
...
function renderGraphFocusContextPanel({ focusedNoteId = "", nodeMap = new Map(), edges = [] } = {}) {
  ...
  directEdges.forEach((edge) => {
    ...
    if (!grouped.has(visual.key)) grouped.set(visual.key, []);
    grouped.get(visual.key).push(edge);
  });
  ...
  const nextHint =
    contextMode.key === "writing"
      ? flowCount
        ? "这条笔记已经进入写作路径，优先检查这些前提、后续和草稿连接是否能串成段落。"
        : bridgeCount
          ? "这条笔记已经有桥接线索，下一步可以把其中一条过渡关系补成真正的写作段落。"
          : supportCount
            ? "它已经有支持链，但还没明显进入写作流程，可以挑一条支持关系先转成段落顺序。"
            : "当前写作入口还不明显，先补一条桥接或进入草稿关系，会比继续堆节点更有用。"
      : conflictCount
        ? "这条笔记已经有反方或张力，可以继续检查这些冲突是不是写清了条件。"
        : boundaryCount
          ? "这条笔记已经开始收边界，下一步可以补反方或反例，让判断更稳。"
          : bridgeCount
            ? "这条笔记已经有桥接线索，适合把过渡思路补成完整段落。"
            : supportCount
              ? "它已经有支持链，但还缺边界或反方，读起来会偏单边。"
              : "这条笔记周围还缺清楚的支持或张力关系，可以先补一条真正有理由的连接。";
  ...
  const sections = groupOrder
    .map((key) => {
      ...
      if (!items.length) return "";
      const meta = GRAPH_RELATION_GROUP_META[key] || GRAPH_RELATION_GROUP_META.neutral;
      return `
        <section class="graph-focus-section">
          ...
          <div class="graph-focus-list">
            ${items
              .map((edge) => {
                const { counterpartId, counterpartTitle } = graphFocusedCounterpartTitle(edge, focusedNoteId, nodeMap);
                const rationale = String(edge?.rationale || "").trim();
                const relationLabel = graphRelationTypeLabel(edge?.relationType);
                const direction = graphFocusedEdgeDirection(edge, focusedNoteId);
                const sourceNoteId = String(edge?.fromNoteId || "").trim();
                const relationType = String(edge?.relationType || "").trim().toLowerCase();
                ...
                return `
                  <div class="graph-focus-card">
                    ...
                    <button class="graph-focus-card-action" type="button" data-open-note="${escapeHtml(sourceNoteId)}" data-graph-followup-action="${escapeHtml(actionMeta.action)}"${String(edge?.id || "").trim() ? ` data-graph-relation-id="${escapeHtml(String(edge.id || "").trim())}"` : ""}${String(edge?.toNoteId || "").trim() ? ` data-graph-target-note="${escapeHtml(String(edge.toNoteId || "").trim())}"` : ""}${relationType ? ` data-graph-relation-type="${escapeHtml(relationType)}"` : ""}>${escapeHtml(actionMeta.label)}</button>
                  </div>
                `;
              })
              .join("")}
    ...
    <aside id="graphFocusContextPanel" class="graph-focus-context" aria-label="当前笔记关系上下文">
      <div class="graph-focus-summary">
        <div class="graph-focus-panel-head">
          <div class="graph-focus-kicker">中心阅读</div>
          <button class="graph-overlay-close graph-focus-panel-close" type="button" data-graph-focus-context-toggle="close" aria-label="收起右侧阅读" title="收起右侧阅读">${renderGraphIcon("close")}</button>
        </div>
        <strong>${escapeHtml(focusedTitle)}</strong>
        <span>直接关系 ${directEdges.length} 条。这里把当前笔记周围最值得读的关系按语义拆开，不用再在整张图里找。</span>
      </div>
      <div class="graph-context-mode" aria-label="中心阅读模式">
        ${["argument", "writing"]
          .map((value) => {
      ...
      <div class="graph-focus-metrics">
        <span>支持 ${supportCount}</span>
        <span>张力 ${conflictCount}</span>
        <span>边界 ${boundaryCount}</span>
        <span>桥接 ${bridgeCount}</span>
        <span>过程 ${flowCount}</span>
        ${neutralCount ? `<span>线索 ${neutralCount}</span>` : ""}
        ${indexCount ? `<span>索引 ${indexCount}</span>` : ""}
      </div>
      <div class="graph-focus-next">${escapeHtml(nextHint)}</div>
      ${sections || `<div class="graph-empty">当前这条笔记周围还没有直接关系。先建立一条支持、限定或桥接关系，再回来看图谱。</div>`}
    </aside>
  `;
}
...
function applyGraphThinkingHoverState(thinkingElement) {
  ...
  if (card) {
    ...
    card.innerHTML = `
      ...
      <span>已在图中标出 ${escapeHtml(String(nodeIds.size))} 个节点${edgeCount ? `、${escapeHtml(String(edgeCount))} 条关系` : ""}${detail ? ` · ${escapeHtml(detail)}` : ""}</span>
    `;
  }
}
...
function buildGraphQuestionSpotSummary({ reviewQueueTotal = 0, bridgeGaps = [], conflictCount = 0, aiAnalysis = null } = {}) {
  ...
  const categories = [
    ...
    { key: "isolated", label: "孤立笔记", count: isolatedCount }
  ].filter((item) => Number(item.count || 0) > 0);
  const total = categories.reduce((sum, item) => sum + Number(item.count || 0), 0);
  const detail = categories.length
    ? categories
  ...
  return {
    total,
    label: total ? `${total} 个追问` : "暂无追问",
    detail: categories.length ? `建议先追问：${detail}` : detail,
    categories,
    artifactCount
  };
}
...
function buildGraphThinkingItems({ nodes = [], edges = [], bridgeGaps = [], reviewQueue = null, conflictItems = [], conflictingRelations = [], aiAnalysis = null } = {}) {
  const nodeMap = new Map((Array.isArray(nodes) ? nodes : []).map((node) => [String(node?.id || ""), node]).filter(([id]) => id));
  const analysis = aiAnalysis?.analysis || null;
  const items = [];
  const addItem = (item) => {
  ...
  (Array.isArray(bridgeGaps) ? bridgeGaps : []).filter((gap) => Array.isArray(gap?.noteIds) && gap.noteIds.length).slice(0, 5).forEach((gap, index) => {
    ...
    addItem({
      ...
      title: sourceTitle,
      meta: targetTitle ? `建议连接到「${targetTitle}」` : "孤立笔记",
  ...
  (Array.isArray(conflictItems) ? conflictItems : []).slice(0, 3).forEach((item, index) => {
    ...
    addItem({
      ...
      question: "这里的张力是在提示真实反方，还是概念命名还不够清楚？",
  ...
  (Array.isArray(conflictingRelations) ? conflictingRelations : []).slice(0, 3).forEach((edge, index) => {
    ...
    addItem({
      ...
      detail: String(edge.rationale || "这条关系可能保留了反方、限定或边界条件。").trim(),
  ...
  (Array.isArray(analysis?.isolatedNotes) ? analysis.isolatedNotes : []).slice(0, 5).forEach((note, index) => {
    ...
    addItem({
      ...
      tone: "isolated",
      kicker: "孤立笔记",
      title: String(note?.title || graphThinkingNoteTitle(nodeMap, noteId, "孤立笔记")).trim() || "孤立笔记",
      meta: "暂未进入主题星系",
      detail: String(note?.thesis || "判断它应当桥接到现有主题，还是先留在暂存。").trim(),
      question: "它暂时游离，是因为真的独特，还是因为还没有写出关系理由？",
      actionLabel: "整理",
      actionAttrs: `data-graph-select-isolated="${escapeHtml(isolatedKey)}"`,
      highlightNodeIds: [noteId]
    });
  });

  (Array.isArray(analysis?.relationCandidates) ? analysis.relationCandidates : []).filter((candidate) => !candidate?.componentBridge).slice(0, 4).forEach((candidate, index) => {
    const sourceTitle = graphThinkingNoteTitle(nodeMap, candidate.fromNoteId, "源笔记");
    const targetTitle = graphThinkingNoteTitle(nodeMap, candidate.toNoteId, "目标笔记");
    addItem({
      id: `candidate-${String(candidate?.id || candidate.fromNoteId || index)}-${String(candidate.toNoteId || "")}`,
      priority: 60 - index,
      view: "organize",
      tone: "review",
      kicker: "关系候选",
      title: `${sourceTitle} -> ${targetTitle}`,
      meta: `${graphRelationTypeLabel(candidate.relationType)} · 待确认`,
      detail: String(candidate.rationale || "本地图谱扫描发现两条笔记可能有关联，需要人工判断。").trim(),
      question: "这条候选关系能不能说清“为什么相连”，还是只是标题相似？",
      actionLabel: "查看源笔记",
      actionAttrs: `data-open-note="${escapeHtml(candidate.fromNoteId || "")}"`,
      highlightNodeIds: [candidate.fromNoteId, candidate.toNoteId]
    });
  });
...
function renderGraphThinkingPanelContent({ summary = {}, items = [], includeSummary = true } = {}) {
  ...
  const filterButtons = ["all", "theme", "organize"]
    ...
    .join("");
  return `
    <div class="graph-thinking-filters" aria-label="追问筛选">
      ${filterButtons}
    </div>
    ${
      includeSummary
...
function renderGraphThinkingPanel({ summary = {}, items = [] } = {}) {
  return `
    <aside class="graph-thinking-panel" aria-label="追问">
      <div class="graph-thinking-panel-head">
        <div>
          <strong>${escapeHtml(summary.label || "追问")}</strong>
          <span>${escapeHtml(summary.detail || "按需查看主题、孤立、桥接和关系复核线索。")}</span>
        </div>
        <button class="graph-overlay-close graph-thinking-panel-close" type="button" data-graph-thinking-close aria-label="关闭追问面板" title="关闭追问">${renderGraphIcon("close")}</button>
      </div>
...
function renderGraphPanel() {
  ...
  const reviewQueueTotal = Number(scopedReviewQueue?.total || 0);
  ...
  const isolatedNotes = !showingFocusedNote ? graphComputedIsolatedNotes(scopedAllNodes, scoped.edges, aiIsolatedNotes) : [];
  ...
  if (graphState.loading) {
    notices.push(
      renderGraphInlineNotice({
        tone: "info",
        title: "正在刷新图谱",
        message: `${lastLoadedAtLabel ? `当前先保留 ${lastLoadedAtLabel} 的结果，` : ""}新的节点和关系读完后会自动替换。`
      })
    );
  }
  if (graphState.error) {
  ...
  summary.textContent = graphState.error
    ? `${baseSummary} 这次刷新失败，已保留上一版图谱。`
    : graphState.loading
      ? `${baseSummary} 正在刷新最新结果。`
      : baseSummary;
  const questionSpotSummary = buildGraphQuestionSpotSummary({
    reviewQueueTotal,
    bridgeGaps,
  ...
  const supplementalSections = !showingFocusedNote
    ? `
      ...
      ${renderRelationReviewQueueSection(scopedReviewQueue, { open: graphState.sectionOpen["review-queue"] === true })}
...
function openGraphCandidateRelation(button = null) {
  ...
  const relationType = String(button?.getAttribute?.("data-graph-relation-type") || "").trim().toLowerCase();
  ...
  const opened = openGraphFollowupNote(noteId, relationType === "bridges" ? "bridge" : "relations", {
    targetNoteId,
    relationType,
    rationaleDraft,
...
async function createGraphThemeIndexFromNoteIds(noteIds = [], { title = "", source = "graph-theme-index" } = {}) {
  ...
  if (requestedIds.length < 2) {
    setStatus("至少选择两条永久笔记，才能创建主题笔记", "warn");
    return null;
  }
  ...
  if (eligibleIds.length < 2) {
    setStatus("这组笔记里可用于主题笔记的永久笔记不足两条", "warn");
    return null;
  }
  ...
  const card = await createIndexCard({
    ...
    indexType: "topic",
    title: cleanTitle,
    summary: "从关联图谱保存的主题笔记入口，用于继续提炼中心问题、关系理由和后续写作。",
  ...
  addSystemMessage({
    ...
    body: canEnterWriting
      ? `“${cleanTitle}”已包含 ${eligibleIds.length} 条笔记，其中 ${writingEligibleIds.length} 条可直接进入写作。`
  ...
  setStatus(`已创建主题笔记：${cleanTitle}`, "ok", { priority: 3, holdMs: 4200 });
  renderGraphPanel();
  return card;
}
...
async function createGraphThemeIndexFromButton(button = null) {
  ...
  if (!noteIds.length) {
    setStatus("当前范围没有可创建主题笔记的笔记", "warn");
    return null;
  }
  ...
  try {
    ...
    setStatus(`创建主题笔记失败：${String(error?.message || error)}`, "bad");
    return null;
  } finally {
...
function openGraphFollowupNote(noteId = "", action = "", options = {}) {
  ...
  setStatus("已从图谱打开笔记", "ok", followupStatusOptions);
  return true;
}
  ...
  if (cleanAction === "relations-edit") {
    return withVariants("argument", [
      ...
      },
      {
        ...
        insightQuestionDraft: `如果把这条${relLabel}变成界面提示或流程设计，最该补哪一个判断节点？`
      }
    ]);
  }
...
$("settingsAiLocalModel")?.addEventListener("change", async (event) => {
  const requested = String(event?.target?.value || "").trim();
...
$("graphCanvas")?.addEventListener("click", async (event) => {
  ...
  if (graphRelationCandidateButton) {
    openGraphCandidateRelation(graphRelationCandidateButton);
    return;
  }
  ...
  if (isolatedAction) {
    const noteId = String(isolatedAction.getAttribute("data-open-note") || "").trim();
  ...
  if (relationAdjustment) {
    const adjustment = String(relationAdjustment.getAttribute("data-graph-relation-adjustment") || "").trim().toLowerCase();
    const noteId = relationAdjustment.getAttribute("data-open-note");
    const relationId = relationAdjustment.getAttribute("data-graph-relation-id");
    const action = relationId ? "relations-edit" : "relations";
    openGraphFollowupNote(noteId, action, {
      relationId,
      targetNoteId: relationAdjustment.getAttribute("data-graph-target-note"),
      relationType: relationAdjustment.getAttribute("data-graph-relation-type")
    });
    const labels = {
      strengthen: "补清关系理由",
      "change-type": "判断是否改关系类型",
      reverse: "判断是否需要反向",
      split: "判断是否拆成两条关系",
      remove: "判断是否删除或降级为线索"
    };
    setStatus(`已打开关系编辑：${labels[adjustment] || "整理这条关系"}`, "ok", { priority: 3, holdMs: 3600, requireModule: "explorer" });
    return;
  }
  ...
  if (thinkingToggle) {
    ...
    renderGraphPanel();
    setStatus(graphState.thinkingPanelOpen ? "已打开追问" : "已收起追问", "ok");
    return;
  }
  ...
  if (thinkingVisibilityToggle) {
    ...
    renderGraphPanel();
    setStatus(graphState.thinkingPanelVisible ? "已显示追问" : "已隐藏追问", "ok");
    return;
  }
  ...
  if (thinkingFilter) {
    ...
    renderGraphPanel();
    setStatus(`追问已切换为：${graphThinkingFilterMeta(graphState.thinkingFilter).label}`, "ok");
    return;
  }
  ...
  if (focusDepthButton) {
    ...
    setStatus(`中心阅读已切换到 ${meta.label}`, "ok");
    return;
  }
  ...
  if (focusContextToggle) {
    ...
    graphState.focusContextCollapsed =
      action === "open" ? false : action === "close" ? true : graphState.focusContextCollapsed !== true;
    renderGraphPanel();
    setStatus(graphState.focusContextCollapsed ? "已收起右侧阅读" : "已显示右侧阅读", "ok");
    return;
  }
  ...
  if (contextModeButton) {
    ...
    setStatus(`右侧阅读模式已切换到 ${meta.label}`, "ok");
    return;
  }
  ...
  if (graphNode) {
    ...
    if (isolatedKey) {
      ...
      setStatus(`已打开孤立笔记整理：${String(graphNode.getAttribute("data-node-title") || nodeId).trim() || nodeId}`, "ok");
      return;
    }
    if (nodeId) {
      openGraphSelection({ kind: "node", nodeId });
...
$("graphCanvas")?.addEventListener("click", (event) => {
  ...
  graphState.researchNavigatorTouched = false;
  renderGraphPanel();
  setStatus(`图谱阅读模式已切换为：${mode === "structure" ? "结构图" : "论证图"}`, "ok");
});
