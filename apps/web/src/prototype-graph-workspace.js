    ...
    <section class="graph-relation-workspace" aria-label="已保存的关系">
      ...
      <section class="graph-theme-index-workspace" aria-label="主题整理">
        <div>
          ...
          <p>${escapeHtml(themeNoteIds.length >= 3 ? `把“${sourceTitle}”和相邻 ${themeNoteIds.length - 1} 条笔记整理成一个可写主题，方便之后提炼中心问题。保存后会形成主题索引笔记，方便追溯。` : "先至少保存两条相关关系，再考虑整理成可写主题。")}</p>
        </div>
...
export function renderGraphThemeIndexWorkspace(noteIds = [], { title = "可写主题推荐", relationCount = 0, tone = "", deps = {} } = {}) {
  ...
  return `
    <section class="graph-theme-index-workspace is-${escapeHtml(String(tone || "candidate").trim() || "candidate")}" aria-label="主题整理">
      <div>
        <strong>整理成可写主题</strong>
        <p>${escapeHtml(canCreate ? `这会把 ${cleanNoteIds.length} 条相关笔记收成一个可写主题；之后再提炼中心问题和写作方向。保存后会形成主题索引笔记，方便追溯。` : "先至少凑齐 3 条相关永久笔记，再整理成可写主题。")}</p>
      </div>
      <button class="graph-selection-action is-primary is-theme" type="button" data-graph-create-theme-index data-graph-theme-note-ids="${escapeHtml(cleanNoteIds.join(","))}" data-graph-theme-title="${escapeHtml(cleanTitle)}"${canCreate ? "" : " disabled"}>创建可写主题</button>
    </section>
  `;
}
