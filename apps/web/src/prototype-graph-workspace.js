  ...
  return `
    <section class="graph-relation-workspace" aria-label="已保存的关系">
      ...
      <section class="graph-theme-index-workspace" aria-label="主题整理">
        <div>
          <strong>创建主题索引笔记</strong>
          <p>${escapeHtml(themeNoteIds.length >= 3 ? `把“${sourceTitle}”和相邻 ${themeNoteIds.length - 1} 条笔记整理成主题索引笔记：先写主题问题、关键永久笔记、每条为什么重要和下一步可以写什么。` : "先至少保存两条相关关系，再考虑整理成主题索引笔记。")}</p>
        </div>
        <button class="graph-selection-action is-secondary" type="button" data-graph-create-theme-index data-graph-theme-note-ids="${escapeHtml(themeNoteIds.join(","))}" data-graph-theme-title="${escapeHtml(themeTitle)}"${themeNoteIds.length >= 3 ? "" : " disabled"}>创建主题索引</button>
      </section>
    </section>
  `;
}
...
export function renderGraphThemeIndexWorkspace(noteIds = [], { title = "可写主题推荐", relationCount = 0, tone = "", deps = {} } = {}) {
  ...
  return `
    <section class="graph-theme-index-workspace is-${escapeHtml(String(tone || "candidate").trim() || "candidate")}" aria-label="主题整理">
      <div>
        <strong>创建主题索引笔记</strong>
        <p>${escapeHtml(canCreate ? `这会把 ${cleanNoteIds.length} 条相关笔记收成主题索引笔记；保存主题问题、关键永久笔记、每条为什么重要和下一步可以写什么。` : "先至少凑齐 3 条相关永久笔记，再整理成主题索引笔记。")}</p>
      </div>
      <button class="graph-selection-action is-primary is-theme" type="button" data-graph-create-theme-index data-graph-theme-note-ids="${escapeHtml(cleanNoteIds.join(","))}" data-graph-theme-title="${escapeHtml(cleanTitle)}"${canCreate ? "" : " disabled"}>创建主题索引</button>
    </section>
  `;
}
