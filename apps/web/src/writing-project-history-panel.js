export function renderWritingProjectHistoryDom({
  ...
  if (projectsHint) {
    const filterSummary = [
      ...
      .filter(Boolean)
      .join("，");
    if (writingState.loadingProjects && writingState.projects.length) projectsHint.textContent = `正在刷新最近写作... 当前显示 ${writingState.projects.length} 个主题。`;
    else if (writingState.loadingProjects) projectsHint.textContent = "正在读取最近写作...";
    else if (writingState.projects.length) projectsHint.textContent = `${filterSummary ? `${filterSummary}，` : ""}共找到 ${writingState.projects.length} 个可写主题。`;
    else if (filterSummary) projectsHint.textContent = `${filterSummary}，但暂时没有匹配主题。`;
    else if (!hasProject && projectEntry.projectId) projectsHint.textContent = `当前相关笔记状态：${projectEntry.status}。${projectEntry.hint}`;
    else projectsHint.textContent = `当前相关笔记状态：${projectEntry.status}。${projectEntry.hint}`;
  }
  if (projectsList) {
    if (writingState.loadingProjects) {
      projectsList.innerHTML = writingState.projects.length
        ...
        : `<div class="writing-empty">正在加载最近写作...</div>`;
      ...
      projectsList.innerHTML = !hasProject && projectEntry.projectId
        ? `<div class="writing-empty">当前相关笔记已经对应 ${escapeHtml(projectEntry.status)}。直接用上面的“${escapeHtml(projectEntry.actionLabel)}”继续，会比重新开始更连续。</div>`
        : `<div class="writing-empty">当前相关笔记状态：${escapeHtml(projectEntry.status)}。${escapeHtml(projectEntry.hint)}</div>`;
    }
  }
    ...
    if (!writingState.project?.id) {
      scaffoldVersionsHint.textContent =
        projectEntry?.projectId && projectEntry?.actionLabel
          ? `当前相关笔记已经对应${projectEntry.status}。先用上面的“${projectEntry.actionLabel}”继续，这里就会显示当前主题的文章提纲版本。`
          : `当前相关笔记状态：${projectEntry.status}。${projectEntry.hint}`;
    }
    else if (writingState.loadingScaffoldVersions && writingState.scaffoldVersions.length) {
      scaffoldVersionsHint.textContent = `正在刷新文章提纲版本... 当前显示 ${writingState.scaffoldVersions.length} 个版本。`;
    } else if (writingState.loadingScaffoldVersions) scaffoldVersionsHint.textContent = "正在读取文章提纲版本...";
    else if (writingState.scaffoldVersions.length) scaffoldVersionsHint.textContent = `当前主题共有 ${writingState.scaffoldVersions.length} 个文章提纲版本。`;
    else scaffoldVersionsHint.textContent = "当前主题还没有文章提纲版本。";
  }
  if (scaffoldVersionsList) {
    if (!writingState.project?.id) {
      scaffoldVersionsList.innerHTML =
        projectEntry?.projectId && projectEntry?.actionLabel
          ? `<div class="writing-empty">当前相关笔记已经对应${escapeHtml(projectEntry.status)}。先用上面的“${escapeHtml(projectEntry.actionLabel)}”继续，这里就会显示当前主题的历史文章提纲版本。</div>`
          : `<div class="writing-empty">当前相关笔记状态：${escapeHtml(projectEntry.status)}。${escapeHtml(projectEntry.hint)}</div>`;
      ...
      scaffoldVersionsList.innerHTML = `<div class="writing-empty">还没有文章提纲版本。点击“生成文章提纲”后会开始累积版本。</div>`;
    }
  }
  ...
  if (draftVersionsHint) {
    if (!writingState.project?.id) {
      draftVersionsHint.textContent =
        projectEntry?.projectId && projectEntry?.actionLabel
          ? `当前相关笔记已经对应${projectEntry.status}。先用上面的“${projectEntry.actionLabel}”继续，这里就会显示当前主题的草稿版本。`
          : `当前相关笔记状态：${projectEntry.status}。${projectEntry.hint}`;
    }
    ...
    else if (writingState.draftVersions.length) draftVersionsHint.textContent = `当前主题共有 ${writingState.draftVersions.length} 个草稿版本。`;
    else draftVersionsHint.textContent = "当前主题还没有草稿版本。";
  }
  if (draftVersionsList) {
    if (!writingState.project?.id) {
      draftVersionsList.innerHTML =
        projectEntry?.projectId && projectEntry?.actionLabel
          ? `<div class="writing-empty">当前相关笔记已经对应${escapeHtml(projectEntry.status)}。先用上面的“${escapeHtml(projectEntry.actionLabel)}”继续，这里就会显示当前主题的草稿版本。</div>`
          : `<div class="writing-empty">当前相关笔记状态：${escapeHtml(projectEntry.status)}。${escapeHtml(projectEntry.hint)}</div>`;
