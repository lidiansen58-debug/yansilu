...
function renderLocalStartStopButtons({ $, ai, localSetupActive, runtimeAvailable, runtimeBusy }) {
  ...
  if (startButton) {
    ...
    startButton.textContent = ai.localRuntimeStarting ? "正在启动..." : "启动本地 AI";
  }
  ...
  if (stopButton) {
    ...
    stopButton.textContent = ai.localRuntimeStopping ? "正在停止..." : managedStopPending ? "继续停止" : "停止本地 AI";
  }
}
...
export function renderAiProviderConfigControlsForRuntime(deps = {}) {
  ...
  if (remoteModelInput) {
    ...
    remoteModelInput.placeholder = remoteConfigurable ? "服务商给你的模型名" : "当前不需要填写远程模型";
  }
...
function renderProviderButtons({ $, ai, providerId, defaultProviderEndpointUrl, defaultProviderHealthEndpointUrl }) {
  ...
  if (saveButton) {
    ...
    saveButton.textContent = ai.providerConfigSaving ? "保存中..." : platformManaged ? "默认远程模型无需保存" : "保存远程设置";
  }
