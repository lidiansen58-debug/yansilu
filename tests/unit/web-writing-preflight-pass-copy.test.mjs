...
test("writing preflight pass messages use Chinese copy", async () => {
  ...
  assert.match(source, /相关笔记里的每条笔记都已经完成一句话判断与三句话提纯确认。/);
  assert.match(source, /相关笔记里没有明显的提纯过短、重复或缺边界问题。/);
  assert.match(source, /\$\{notesWithBoundary\.length\} 条相关笔记已经带有反方或边界。/);
