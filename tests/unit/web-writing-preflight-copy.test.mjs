...
test("writing preflight scaffold warnings use 草稿骨架 wording", async () => {
  const source = await readWritingEngineSource();

  assert.match(source, /已经可以组织成草稿骨架/);
  assert.match(source, /再继续推进草稿骨架/);
  assert.match(source, /让草稿骨架有明确目标/);
  assert.match(source, /让草稿骨架有可复用的问题上下文/);
  assert.match(source, /当前没有阻塞项，可以继续生成草稿骨架/);
