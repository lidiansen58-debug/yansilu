...
async function createAndSaveNoteViaEditor(page, markdown, options = {}) {
  ...
  await waitFor(async () => {
    ...
    assert.match(String(statusText || ""), /已同步|已保存 Markdown|仍按 draft 处理|作者确认/);
  }, 10000);
...
test("prototype literature note can record an original draft through the unified editor flow", async (t) => {
  ...
  await waitFor(async () => {
    ...
    const statusText = await currentStatusText(page);
    assert.match(String(statusText || ""), /已同步|已保存 Markdown/);
  }, 10000);
  ...
  await originalPage.locator("#btnSave").click();
  ...
  await waitFor(async () => {
    ...
    assert.match(String(statusText || ""), /已同步|仍按 draft 处理|已保存 Markdown/);
  }, 10000);
});
...
test("prototype editor tab indents and shift-tab outdents selected lines", async (t) => {
  ...
  await waitFor(async () => {
    const status = await currentStatusText(page);
    assert.match(String(status || ""), /已同步|同步|已保存 Markdown/);
  }, 10000);
...
test("prototype editor shows dirty state and supports Ctrl/Cmd+S save", async (t) => {
  ...
  await waitFor(async () => {
    ...
    assert.match(String(saveButtonTip || ""), /保存/);
  }, 7000);
...
test("prototype editor keeps content editable when toggling source and wysiwyg with related panel open", async (t) => {
  ...
  const notes = await waitFor(async () => {
    ...
    assert.ok(matched, JSON.stringify(result.json.items, null, 2));
    const status = await currentStatusText(page);
    assert.match(String(status || ""), /已同步|同步|已保存 Markdown|仍按 draft 处理/);
    return matched;
  }, 10000);
