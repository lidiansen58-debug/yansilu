...
test("paper workspace browser flow preserves draft, selection, failure, and permanent-note continuity across reload", async (t) => {
  ...
  const unsavedBoundary = "This draft is not ready to save yet.";
    ...
    await waitFor(async () => {
      ...
      assert.match(String(statusText || ""), /å·²è¿žæŽ¥ API/);
    }, 4000);
    ...
    await waitFor(async () => {
      ...
      const statusText = await currentPaperWorkspaceStatusText(page);
      assert.match(String(statusText || ""), /è®ºæ–‡å·¥ä½œå°å·²åˆ›å»º/);
    }, 6000);
    await waitFor(async () => {
      ...
      assert.match(String(translationStepText || ""), /è¿˜æ²¡æœ‰å€™é€‰/);
      assert.match(String(translationStepText || ""), /ç²˜è´´ NotebookLM è¾“å‡ºåŽï¼Œè¿™é‡Œä¼šå…ˆç”Ÿæˆ literature å€™é€‰ï¼Œè€Œä¸æ˜¯ç›´æŽ¥ç”Ÿæˆæ°¸ä¹…ç¬”è®°/);
      assert.match(String(translationStepText || ""), /å°šæœªé€‰æ‹©å€™é€‰/);
      assert.match(String(translationStepText || ""), /å…ˆä»Žå·¦ä¾§é€‰ä¸€æ¡å€™é€‰/);
    }, 4000);
  ...
  await page.fill(
    ...
    await waitFor(async () => {
      ...
      const statusText = await currentPaperWorkspaceStatusText(page);
      assert.match(String(statusText || ""), /NotebookLM å†…å®¹å·²è½¬æˆ literature å€™é€‰/);
    }, 6000);
  ...
  await waitFor(async () => {
    ...
    const translationStepText = await page.locator(".paper-grid .paper-card.paper-span-2").nth(0).textContent();
    assert.match(String(translationStepText || ""), /å…ˆä¿å­˜è¿™æ¡å€™é€‰çš„ç”¨æˆ·è½¬è¿°ï¼Œå†è¿›å…¥æ°¸ä¹…ç¬”è®°å€™é€‰/);
    assert.match(String(translationStepText || ""), /å…³ç³»å’Œè¾¹ç•Œä¿¡æ¯ä¹Ÿä¼šä¸€èµ·æ¢å¤/);
  }, 4000);
  ...
  await waitFor(async () => {
    ...
    await waitFor(async () => {
      ...
      const statusText = await currentPaperWorkspaceStatusText(page);
      assert.match(String(statusText || ""), /ç”¨æˆ·è½¬è¿°å·²ä¿å­˜/);
    }, 6000);
    await waitFor(async () => {
      ...
      assert.match(String(permanentStepText || ""), /è¿˜æ²¡æœ‰æ°¸ä¹…ç¬”è®°å€™é€‰/);
      assert.match(String(permanentStepText || ""), /å…ˆåœ¨ Step 3 ä¿å­˜è½¬è¿°å¹¶ç”Ÿæˆå€™é€‰/);
      assert.match(String(permanentStepText || ""), /ä¿å­˜è½¬è¿°åŽï¼Œå¯ä»¥ä¸ºå½“å‰å€™é€‰ç”Ÿæˆæ°¸ä¹…ç¬”è®°å€™é€‰/);
      assert.match(String(permanentStepText || ""), /å€™é€‰åªæ˜¯ä¸€ä»½è‰ç¨¿éª¨æž¶/);
      assert.match(String(permanentStepText || ""), /ç¡®è®¤ authorship ä¹‹åŽæ‰ä¼šçœŸæ­£ä¿å­˜ä¸ºæ°¸ä¹…ç¬”è®°/);
    ...
    await waitFor(async () => {
      ...
      assert.notEqual(await permanentCandidateButton.getAttribute("disabled"), null);
      const statusText = await currentPaperWorkspaceStatusText(page);
      assert.match(String(statusText || ""), /å·²é€‰æ‹©å€™é€‰/);
    }, 4000);
    ...
    await waitFor(async () => {
      ...
      const statusText = await currentPaperWorkspaceStatusText(page);
      assert.match(String(statusText || ""), /æ“ä½œå¤±è´¥/);
      assert.equal(await page.locator("#translationParaphraseInput").inputValue(), "");
      ...
      assert.match(String((await page.locator("#btnSaveTranslation").textContent()) || ""), /ä¿å­˜è½¬è¿°/);
      assert.notEqual(await permanentCandidateButton.getAttribute("disabled"), null);
      const translationStepText = await page.locator(".paper-grid .paper-card.paper-span-2").nth(0).textContent();
      assert.match(String(translationStepText || ""), /å·²æ¢å¤è¿™æ¡å€™é€‰çš„æœ¬åœ°æœªä¿å­˜è½¬è¿°è‰ç¨¿/);
    }, 6000);
    ...
    await waitFor(async () => {
      ...
      const translationStepText = await page.locator(".paper-grid .paper-card.paper-span-2").nth(0).textContent();
      assert.match(String(translationStepText || ""), /å·²æ¢å¤è¿™æ¡å€™é€‰çš„æœ¬åœ°æœªä¿å­˜è½¬è¿°è‰ç¨¿/);
      assert.match(String(translationStepText || ""), /å¯ä»¥ç»§ç»­ä¿®æ”¹åŽå†ä¿å­˜/);
    }, 6000);
  ...
  await waitFor(async () => {
    ...
    assert.equal(await permanentCandidateButton.getAttribute("disabled"), null);
    const translationStepText = await page.locator(".paper-grid .paper-card.paper-span-2").nth(0).textContent();
    assert.match(String(translationStepText || ""), /ä½ å¯ä»¥ç»§ç»­ä¿®æ”¹ï¼Œä¹Ÿå¯ä»¥ç›´æŽ¥ç”Ÿæˆæ°¸ä¹…ç¬”è®°å€™é€‰/);
  }, 4000);
  ...
  await waitFor(async () => {
    ...
    await waitFor(async () => {
      ...
      assert.equal(await permanentCandidateButton.getAttribute("disabled"), null);
      const statusText = await currentPaperWorkspaceStatusText(page);
      assert.match(String(statusText || ""), /å·²æ¢å¤è¿™æ¡å€™é€‰çš„ç”¨æˆ·è½¬è¿°/);
      const translationStepText = await page.locator(".paper-grid .paper-card.paper-span-2").nth(0).textContent();
      assert.match(String(translationStepText || ""), /è¿™æ¡å€™é€‰å·²ç»ä¿å­˜è¿‡è½¬è¿°/);
    }, 4000);
      ...
      await waitFor(async () => {
        ...
        assert.match(String(translationStepText || ""), /å·²æ¢å¤è¿™æ¡å€™é€‰çš„æœ¬åœ°æœªä¿å­˜è½¬è¿°è‰ç¨¿/);
          ...
          await waitFor(async () => {
            ...
            const translationStepText = await page.locator(".paper-grid .paper-card.paper-span-2").nth(0).textContent();
            assert.match(String(translationStepText || ""), /è¿™æ¡å€™é€‰å·²ç»ä¿å­˜è¿‡è½¬è¿°/);
            assert.match(String(translationStepText || ""), /ä¹Ÿå¯ä»¥ç›´æŽ¥ç”Ÿæˆæ°¸ä¹…ç¬”è®°å€™é€‰/);
          }, 4000);
          ...
          await waitFor(async () => {
            ...
            const statusText = await currentPaperWorkspaceStatusText(page);
            assert.match(String(statusText || ""), /æ°¸ä¹…ç¬”è®°å€™é€‰å·²ç”Ÿæˆ/);
            assert.equal(await page.locator("#confirmAuthorshipInput").isChecked(), false);
        ...
        await waitFor(async () => {
          ...
          assert.match(await page.locator(".paper-permanent-preview").textContent(), /An unsaved draft should survive candidate switches/);
            ...
            await waitFor(async () => {
              ...
              assert.match(String((await page.locator("#btnSavePermanentNote").textContent()) || ""), /ç¡®è®¤ä¿å­˜ä¸ºæ°¸ä¹…ç¬”è®°/);
              const statusText = await currentPaperWorkspaceStatusText(page);
              assert.match(String(statusText || ""), /å·²æ¢å¤è¿™æ¡æ°¸ä¹…ç¬”è®°å€™é€‰/);
            }, 4000);
            ...
            await waitFor(async () => {
              ...
              assert.equal(await page.locator("[data-paper-permanent-candidate-id]").count(), 2);
              const statusText = await currentPaperWorkspaceStatusText(page);
              assert.match(String(statusText || ""), /è®ºæ–‡å·¥ä½œå°å·²è¯»å–/);
            }, 6000);
            await waitFor(async () => {
              const previewText = await page.locator(".paper-permanent-preview").textContent();
              assert.match(String(previewText || ""), /My takeaway is that retrieval effort improves later access to the idea/);
          ...
          await waitFor(async () => {
            ...
            await waitFor(async () => {
              ...
              const statusText = await currentPaperWorkspaceStatusText(page);
              assert.match(String(statusText || ""), /æ°¸ä¹…ç¬”è®°å·²ä¿å­˜/);
            }, 6000);
            await waitFor(async () => {
              const previewText = await page.locator(".paper-permanent-preview").textContent();
              assert.match(String(previewText || ""), /My takeaway is that retrieval effort improves later access to the idea/);
              assert.doesNotMatch(String(previewText || ""), /An unsaved draft should survive candidate switches/);
              assert.doesNotMatch(String(previewText || ""), /An unsaved draft should survive candidate switches/);
              assert.match(String(previewText || ""), new RegExp(`å·²ä¿å­˜ä¸ºï¼š${savedPermanentNoteId}`));
              assert.equal(await page.locator("#confirmAuthorshipInput").isChecked(), true);
              assert.match(String((await page.locator("[data-paper-permanent-candidate-id]").nth(0).getAttribute("class")) || ""), /is-active/);
              assert.doesNotMatch(String((await page.locator("[data-paper-permanent-candidate-id]").nth(1).getAttribute("class")) || ""), /is-active/);
              assert.equal(await page.locator("[data-paper-permanent-candidate-id]").count(), 2);
              assert.notEqual(await page.locator("#btnSavePermanentNote").getAttribute("disabled"), null);
              assert.match(String((await page.locator("#btnSavePermanentNote").textContent()) || ""), /å·²ä¿å­˜ä¸ºæ°¸ä¹…ç¬”è®°/);
            }, 4000);
...
test("prototype AI inbox review-action continuity keeps detail aligned with filtered pending selection changes", async (t) => {
  ...
  await waitFor(async () => {
    ...
    const detailText = await page.locator("#aiInboxPanel .ai-inbox-detail-pane").textContent();
    assert.match(String(listText || ""), /å½“å‰ç­›é€‰ä¸‹æ²¡æœ‰å¾…å¤„ç†å»ºè®®/);
    assert.match(String(detailText || ""), /ä»Žå·¦ä¾§é€‰æ‹©ä¸€æ¡å»ºè®®/);
    assert.doesNotMatch(String(detailText || ""), /æ­£åœ¨è¯»å–å»ºè®®è¯¦æƒ…/);
